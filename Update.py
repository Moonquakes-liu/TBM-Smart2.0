        ...
        with zipfile.ZipFile(os.path.join(root_path, 'temp', 'ROM', 'Resources.zip'), mode='r') as zips:
            ...
            for index, file_zips in enumerate(zips.infolist()):
                ...
                try:
                    for section in parser.sections():
                        if (section != 'program') and (parser.get(section, 'file') == os.path.basename(target_file)):
                            ...
                            if raw_md5 != check_md5:
                                Object.show_info(message=f'File < {file_zips.filename} > verification failure!', Type='error')
                    ...
                    sum_failure += 1
                    continue
                Object.show_info(message=f'Checking file < {file_zips.filename} successful!', Type='info')
                ...
                try:
                    if os.path.exists(target_file):
                        ...
                        Object.show_info(message=f'Backup file < {file_zips.filename} > successful!', Type='info')
                        Object.percentage(value=5 + index * 90 / files_num + 90 / files_num / 3)
                        os.remove(target_file)
                    zips.extract(file_zips.filename, root_path)
                    Object.show_info(message=f'Update file < {file_zips.filename} > successful!', Type='info')
                    Object.percentage(value=5 + index * 90 / files_num + 90 / files_num / 3)
                except Exception:
                    Object.show_info(message=f'Update file < {file_zips.filename} > failure!', Type='error')
                    sum_failure += 1
                sum_successful += 1
